package vip.fubuki.thirst.foundation.config;
...
public class KeyWordConfig {
    ...
    static {
        ...
        KEYWORD_BLACKLIST = BUILDER.define("keyword_blacklist", "(dried|candied|leaf|leaves|gummy|crate|jam|sauce|bucket|seed|cookie|pie|bush|sapling|bean|curry|cake|candy)");
        BUILDER.pop();

        BUILDER.push("Drink KeyWord")
                .comment("Defines items will be considered as drink if it contains the keyword",
                        "Format: [(keyword1|keyword2|keyword3)]");
        KEYWORD_DRINK = BUILDER.define("keyword_drink", "(drink|juice|tea|soda|coffee|wine|beer|cider|yogurt|milkshake|smoothie)");
        BUILDER.pop();

        BUILDER.push("Soup KeyWord")
                .comment("Defines items will be considered as soup if it contains the keyword",
                        "Format: [(keyword1|keyword2|keyword3)]");
        KEYWORD_SOUP = BUILDER.define("keyword_soup", "(soup|stew|porridge)");
        BUILDER.pop();

        BUILDER.push("Fruit KeyWord")
                .comment("Defines items will be considered as fruit if it contains the keyword",
                        "Format: [(keyword1|keyword2|keyword3)]");
        KEYWORD_FRUIT = BUILDER.define("keyword_fruit", "(fruit|berry|berries|grape|orange|peach|pear|coconut|lemon|melon|cherry|apple)");
        BUILDER.pop();
